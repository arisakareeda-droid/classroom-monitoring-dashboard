        ...
        st.markdown(
            ...
            <div style="display:flex;justify-content:flex-end;">
                <div style="display:flex;align-items:center;gap:8px;background:rgba(255,255,255,0.10);
                    ...
                    <span style="text-transform:uppercase;letter-spacing:0.06em;">Offline</span>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
...
try:
    ...
    with st.sidebar:
        ...
        if room_col:
            st.markdown(
                "<div class='sidebar-eyebrow'>เลือกห้องเรียนเพื่อเปรียบเทียบ</div>", unsafe_allow_html=True
            )
            all_rooms = sorted(df[room_col].dropna().unique().tolist())
            ...
            if selected_rooms:
                df = df[df[room_col].isin(selected_rooms)]
            else:
                st.warning("กรุณาเลือกอย่างน้อย 1 ห้องเรียน")
                st.stop()
    ...
    if "Date" in df.columns and "Person Count" in df.columns:
        ...
        st.markdown(
            section_header(
                "แนวโน้มการเข้า-ออกห้องเรียนรวม",
                "Overall Classroom Access · Time Series Analysis",
            ),
            unsafe_allow_html=True,
        )
        ...
        line_fig = px.line(
            ...
            markers=True,
            color_discrete_sequence=[theme["line_color"]],
            labels={"Date": "วันที่", "Person Count": "จำนวนผู้เข้าใช้ห้องเรียนรวม (คน)"},
        )
        line_fig.update_traces(
            line=dict(width=2.8, shape="spline"),
            marker=dict(size=7, color=theme["marker_color"]),
        ...
        # ==================================================
        # กราฟเปรียบเทียบสัดส่วนและความถี่การใช้งานแต่ละห้อง
        # ==================================================
        if room_col:
            st.markdown(
                section_header(
                    "การเปรียบเทียบสัดส่วนและความถี่การใช้งานรายห้อง",
                    "Classroom Usage Proportion & Frequency Comparison",
                ),
                unsafe_allow_html=True,
            )
            ...
            # 1. กราฟวงกลม (Donut Chart) แสดงสัดส่วนการใช้งานรวมของแต่ละห้อง
            with col_c1:
                st.markdown("<div style='font-weight:600; margin-bottom:10px;'>สัดส่วนปริมาณการใช้งานรวม (%) แยกตามห้อง</div>", unsafe_allow_html=True)
                room_share = df.groupby(room_col)["Person Count"].sum().reset_index()
                ...
                st.plotly_chart(style_chart(pie_fig, theme, 380), use_container_width=True)

            # 2. กราฟแสดงความถี่/จำนวนครั้งของการใช้งาน (Record Count) และยอดรวมคน (Total People) เปรียบเทียบแต่ละห้อง
            with col_c2:
                st.markdown("<div style='font-weight:600; margin-bottom:10px;'>ความถี่และปริมาณสะสมเปรียบเทียบแต่ละห้อง</div>", unsafe_allow_html=True)
                room_freq = df.groupby(room_col).agg(
                    Total_People=("Person Count", "sum"),
                    Frequency=("Person Count", "count")
                ).reset_index().sort_values("Total_People", ascending=True)
                ...
                freq_fig = px.bar(
                    ...
                    color="Total_People",
                    color_continuous_scale=theme["bar_scale"],
                    labels={"Total_People": "จำนวนคนสะสมทั้งหมด", room_col: "ห้องเรียน", "Frequency": "ความถี่ (ครั้ง)"}
                )
                freq_fig.update_traces(texttemplate='ความถี่: %{text} ครั้ง', textposition='outside', marker_line_width=0)
                freq_fig.update_layout(coloraxis_showscale=False)
                st.plotly_chart(style_chart(freq_fig, theme, 380), use_container_width=True)

            st.markdown("<br>", unsafe_allow_html=True)

            # 3. กราฟแนวโน้มแยกตามห้อง (Multi-line Chart ตามช่วงเวลา)
            st.markdown(
                section_header(
                    "เปรียบเทียบแนวโน้มการใช้งานรายวันของแต่ละห้อง",
                    "Daily Trend Comparison by Classroom",
                ),
                unsafe_allow_html=True,
            )
            
            room_daily = df.groupby(["Date", room_col])["Person Count"].sum().reset_index()
            multi_line = px.line(
                room_daily,
                x="Date",
                y="Person Count",
                color=room_col,
                markers=True,
                labels={"Date": "วันที่", "Person Count": "จำนวนคน", room_col: "ห้องเรียน"}
            )
            multi_line.update_traces(line=dict(width=2))
            st.plotly_chart(style_chart(multi_line, theme, 400), use_container_width=True)
